...
function resolveCookieMaxAge(session: AuthSession): number {
  const rawExpiry = session.expires_at ? Date.parse(session.expires_at) : Number.NaN;
  if (!Number.isFinite(rawExpiry)) {
    return session.remember_me ? SESSION_MAX_AGE_SECONDS * 5 : SESSION_MAX_AGE_SECONDS;
  }
  return Math.max(60, Math.floor((rawExpiry - Date.now()) / 1000));
}

function setAuthCookie(token: string, session: AuthSession) {
  document.cookie = `${AUTH_COOKIE_NAME}=${encodeURIComponent(token)}; path=/; max-age=${resolveCookieMaxAge(session)}; SameSite=Lax`;
}
...
export function persistAuthSession(session: AuthSession) {
  ...
  setAuthCookie(session.access_token, session);
...
export function verifyEmailToken(token: string) {
  return postJson<AuthSession>("/auth/verify-email", { token });
}
