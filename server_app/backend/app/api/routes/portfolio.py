...
async def get_portfolio(
    ...
    if latest is None:
        ...
        account_id = str(portfolio_snapshot.get("account_id") or latest.account_id)
        broker = str(portfolio_snapshot.get("broker") or latest.broker)
        total_equity = float(portfolio_snapshot.get("total_equity", latest.total_equity) or latest.total_equity)
        cash = float(portfolio_snapshot.get("cash", latest.cash) or latest.cash)
        buying_power = float(portfolio_snapshot.get("buying_power", latest.buying_power) or latest.buying_power)
        daily_pnl = float(portfolio_snapshot.get("daily_pnl", latest.daily_pnl) or latest.daily_pnl)
        weekly_pnl = float(portfolio_snapshot.get("weekly_pnl", latest.weekly_pnl) or latest.weekly_pnl)
        monthly_pnl = float(portfolio_snapshot.get("monthly_pnl", latest.monthly_pnl) or latest.monthly_pnl)
        gross_exposure = float(portfolio_snapshot.get("gross_exposure", latest.gross_exposure) or latest.gross_exposure)
        net_exposure = float(portfolio_snapshot.get("net_exposure", latest.net_exposure) or latest.net_exposure)
        max_drawdown = float(portfolio_snapshot.get("max_drawdown", latest.max_drawdown) or latest.max_drawdown)
        var_95 = float(portfolio_snapshot.get("var_95", latest.var_95) or latest.var_95)
        margin_usage = float(portfolio_snapshot.get("margin_usage", latest.margin_usage) or latest.margin_usage)
        risk_limits = {**dict(latest.risk_limits or {}), **dict(portfolio_snapshot.get("risk_limits") or {})}
