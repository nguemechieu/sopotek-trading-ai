...
class BaseDerivativeBroker(BaseBroker):
    ...
    async def _request_json(
        ...
        *,
        ...
        async with session.request(method.upper(), url, params=params, json=json_payload, data=data, headers=request_headers) as response:
            if response.status not in expected_statuses:
                body = await response.text()
                raise RuntimeError(f"{self.exchange_name} {method.upper()} {url} failed: {response.status} {body}")
